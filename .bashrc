...
alias nb="npm run build; notifyresult 'build'"
alias nd="npm run dist"
alias nls="npm ls"
alias nbw="npm run build -- --watch"
alias l="npm run lint ; notifyresult lint"
alias lint="npm run lint ; notifyresult lint"
alias ll="npm run lint:src; notifyresult lint:src"
alias lf="npm run lint -- --fix ; notifyresult lint"
alias lnt="npm run lint && npm run test ; notifyresult"
alias st="standard"
alias stf="standard --fix"
alias nrd="npm run deploy && pusht ; notifyresult deploy"
alias nrs="npm run deploy:staging2 ; notifyresult deploy:staging2"
alias yds="yarn deploy:staging2; notifyresult deploy:staging2"
...
#   fnm migrate <from-node> <to-node>   e.g. fnm migrate 22 24
# Registry packages are reinstalled with `npm install -g`.
# Linked packages (npm link) are re-linked from ~/projects/<name> using the target Node version.
# All other subcommands are passed through to the real fnm binary.
# Note: defined after the `fnm env` eval above so that command -v fnm still
# resolves to the real binary when that line runs at shell startup.
fnm() {
  if [ "$1" = "migrate" ]; then
    shift
...
_fnm_migrate_globals() {
  local from="${1:-}" to="${2:-}"
  if [ -z "$from" ] || [ -z "$to" ]; then
    echo "Usage: fnm migrate <from-node> <to-node>" >&2
    return 1
  fi

  echo "Reading globals from Node $from..."
  local json
  json="$(command fnm exec --using "$from" npm ls -g --depth=0 --json 2>/dev/null)" || {
    echo "Failed to read globals from Node $from" >&2
    return 1
  }

  local registry=() links=()
  local type name
...
$(echo "$json" | jq -r '
  ...
  | if ((.value.resolved // "") | startswith("file:"))
    then "link\t" + ((.value.resolved | sub("^file:"; "") | split("/") | last))
    else "registry\t" + .key
    end
')
EOF

  if [ ${#registry[@]} -eq 0 ] && [ ${#links[@]} -eq 0 ]; then
    echo "No global packages found."
    return 0
  fi

  if [ ${#registry[@]} -gt 0 ]; then
    echo "Installing registry globals into Node $to:"
    printf '  - %s\n' "${registry[@]}"
    command fnm exec --using "$to" npm install -g "${registry[@]}"
  fi

  if [ ${#links[@]} -gt 0 ]; then
    echo "Re-linking local globals into Node $to:"
    local proj dir
    for proj in "${links[@]}"; do
      dir="$HOME/projects/$proj"
      if [ -d "$dir" ]; then
        echo "==> npm link in $dir"
        ( cd "$dir" && command fnm exec --using "$to" npm link )
      else
        echo "Skipping $proj: $dir not found" >&2
      fi
    done
  fi
}
